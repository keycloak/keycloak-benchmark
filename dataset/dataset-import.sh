...
main () {
  ...
  case "$ACTION" in
    create-realms)
      ...
      execute_command "create-realms?count=$REALM_COUNT&clients-per-realm=$CLIENTS_COUNT&users-per-realm=$USERS_COUNT$HI_PARAM$HA_PARAM&realm-prefix=$REALM_PREFIX"
      exit 0
      ;;
    create-clients)
