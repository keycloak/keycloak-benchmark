...
mkdir -p "logs"
if aws secretsmanager describe-secret --region $REGION --secret-id "$KEYCLOAK_MASTER_PASSWORD_SECRET_NAME" --no-cli-pager > "logs/${CLUSTER_NAME}/$(date -uIs)_describe-secret.log"; then
