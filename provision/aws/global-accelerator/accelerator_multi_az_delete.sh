...
function deleteLoadBalancer() {
  ...
  oc delete -n ${NAMESPACE} svc ${SVC_NAME}
}
...
if [ "${DELETE_LB}" = true]; then
  requiredEnv PRIMARY_CLUSTER BACKUP_CLUSTER KEYCLOAK_NAMESPACE

  deleteLoadBalancer ${PRIMARY_CLUSTER} ${ACCELERATOR_LB_NAME} ${KEYCLOAK_NAMESPACE}
  deleteLoadBalancer ${BACKUP_CLUSTER} ${ACCELERATOR_LB_NAME} ${KEYCLOAK_NAMESPACE}
fi
