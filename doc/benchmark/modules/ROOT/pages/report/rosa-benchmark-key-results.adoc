...
* For each 8 password-based user logins per second, 1 vCPU per Pod in a three-node cluster (tested with up to 50 password-based user logins per second).
+
...
* CPU requested: 4 vCPU
+
(24 logins per second = 3 vCPU, 450 client credential grants per second = 1 vCPU)
