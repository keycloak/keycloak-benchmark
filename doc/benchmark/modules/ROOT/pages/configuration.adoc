...
| A user can provide an increment value to run the incremental benchmark to find the limit of a given system under test for the assertions set within the simulation(s).
If the CLI option is set without a parameter, it defaults to an increment value of `32`.
See xref:kcb-modes.adoc[] for more insight into how the incremental scalability benchmark works.
