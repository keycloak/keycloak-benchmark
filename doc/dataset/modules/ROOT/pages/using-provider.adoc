...
Alternatively, we could also use the link:{github-files}/provision/minikube/Taskfile.yaml[Taskfile] from within the provision/minikube module to execute the `dataset-import` task, to seamlessly execute your automation for K8s style deployments.

The `dataset-task` re-uses the command line arguments defined above to make it convenient for the user.
