...
+
--
* `true` -- Keycloak is configured to use custom configuration file that is mounted to each pod. The file can be found here: `link:{github-files}/provision/minikube/keycloak/config/kcb-infinispan-cache-config.xml[kcb-infinispan-cache-config.xml]`.
* `false` -- use the default Infinispan configuration that is part of Keycloak distribution.
--
