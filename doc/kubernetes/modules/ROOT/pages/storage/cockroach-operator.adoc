...
=== Kubernetes 1.25+ not supported by CockroachDB operator

The operator still uses the resource `PodDisruptionBudget` that has been deprecated in Kubernetes 1.21+ and is unavailable in 1.25+.
Therefore, it can't be used in Kubernetes 1.25+ yet until issue https://github.com/cockroachdb/cockroach-operator/issues/928[cockroachdb/cockroach-operator#928] is resolved.
...
=== Operator can't connect to CockroachDB instances in other namespaces

Issue https://github.com/cockroachdb/cockroach-operator/issues/906[cockroachdb/cockroach-operator#906] describes that the operator can't connect to CockroachDB instances in other namespaces.

Due to that, the installation here has the CockroachDB operator installed in the same namespace as the CockroachDB, which is the namespace `keycloak`.
It only listens to the current namespace as the environment variable `WATCH_NAMESPACE` is set.
