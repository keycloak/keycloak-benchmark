...
The deployment adds a new Infinispan pod to the minikube setup and will remove all other storage pods that are no longer necessary.
...
Assuming that the IP-address is `192.168.39.39`, point the local instance to `192.168.39.39:30011`.

It can be used, for example, with Infinispan's command line client.

[WARNING]
====
The infinispan instance running inside the pod will advertise its pod IP address to the caller that is not accessible from the outside of minikube.

Due to this, accessing Infinispan from an application running outside minikube will most likely fail until the required but yet unknown parameters are passed to either the application or the pod.
====
