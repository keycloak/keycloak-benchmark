...
This is necessary when the primary site has lost its state in {ispn}, a network partition occurred between the primary and the secondary site while the secondary site was active, or the replication was disabled as described in xref:./switch-over.adoc[].

If the data in {ispn} on both sites is still in sync, the procedure for {ispn} can be skipped.
