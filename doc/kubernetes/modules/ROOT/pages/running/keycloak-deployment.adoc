...
Adjust this number to meet your system's needs.
As most requests won't touch the database due to {keycloak}'s embedded cache, this can server several hundreds of requests per second.
...
The number of all {keycloak} threads in the StatefulSet should not exceed the number of JGroup threads to avoid a JGroup thread pool exhaustion which could stall {keycloak} request processing.
