...
WARNING: Ensure you are connected to the secondary site before executing the clear operation.

. Login into your secondary site

. Run the `crossdc-clear-caches` Batch CR (see xref:#running-infinispan-batch[]).

. Delete all Batch CRs created, since they are not required anymore.
