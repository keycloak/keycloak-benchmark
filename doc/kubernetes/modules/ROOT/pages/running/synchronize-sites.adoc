...
this typically occurs after a split-brain or when one site has been taken offline for maintenance.
...
Once the two sites have been synchronized, it's safe to add the previously offline site back to the Global Accelerator
EndpointGroup following the steps in the xref:running/bring-active-site-online.adoc[] guide.

== Further reading

See https://www.{project_name}.org/high-availability/concepts-{jdgserver_name}-cli-batch[Concepts to automate {jdgserver_name} CLI commands]
