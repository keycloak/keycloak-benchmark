...
If metrics from multiple clusters are being collected, this tag helps identifying where do they belong to.
* `node=<node>`: The name of the node reporting the metric.
====

== Response Time

The following metrics exposes the response time for the remote requests.
The response time is measure between two nodes and includes the processing time.
...
Also, all the internal message, as heartbeats, are counted too.
They allow to compute the bandwidth currently used by each node.
...
Monitoring the thread pool size is a good indicator that a node is under heavy load.
All requests received are added to the thread pool for processing and, when it is full, the request is discarded.
A retransmission mechanism ensures a reliable communication with an increase of resources usages.
...
When a value different from zero, it may signal that a receiver is overloaded and may degrade the cluster performance.
...
| The number of times flow control blocks sender for unicast messages.
...
| The number of times flow control blocks sender for multicast messages.
...
Retransmissions increase resources usage, and it is usually a signal of an overload system.
...
| The total number of dropped message by the sender.
...
The value of `2` is used on nodes where the status is unknown; not all node establish connections to the remote sites and do not contain this information.
...
Network partitions in a cluster can happen due to various reason.
...
| The number of time a network split was detected and healed.
