...
These metrics are a good indicator of how frequent the database entities are modified.
====
endif::[]
...
As {project_name} caches the database entities, database access always proceeds with an eviction event.
...
A rapid increase of eviction and very high database usage means the `users` or `realms` cache is too small for smooth {project_name} operation.
If enough memory is available, consider increase the max cache size using the CLI options `cache-embedded-users-max-count` or `cache-embedded-realms-max-count`
====
endif::[]
