...
The following metrics monitors the cache accesses, the reads and writes and their duration.

|===
|Metric |Description

| `vendor_statistics_read_write_ratio`
| The read/write ratio.
A value greater than 1.0 means the cache is more read-intensive.

| `vendor_statistics_hit_ratio`
| The percentages of reads that found the value.
...
Transactional caches uses both One-Phase-Commit and Two-Phase-Commit protocols to complete a transaction.
There metrics keep track of the duration for the operations.

NOTE: The `PESSMISTIC` locking mode uses One-Phase-Commit and does not create commit requests.

TIP: On a healthy cluster, the number of rollback should remain zero.
...
It is required to rebalance the data stored and guarantee the desired number of copies.

This operation increases the resources usage and it will affect negatively the overall performance.
...
|The total number of cross-site handled by this node.
This metric is more detailed with a per-site counter.

| `vendor_x_site_admin_status`
|The site status.
A value of 1 indicates that is online.
